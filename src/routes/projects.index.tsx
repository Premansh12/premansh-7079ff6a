...
function EmptyArchive() {
  return (
    <div className="mt-24 border-y border-border py-24 text-center md:py-32">
      <p className="text-xs uppercase tracking-[0.4em] text-gold">In production</p>
      <h2 className="mt-6 font-serif text-4xl md:text-5xl">
        New case studies, <em className="italic">arriving soon</em>.
      </h2>
      <p className="mx-auto mt-6 max-w-lg text-muted-foreground">
        The archive is being rebuilt with deeper write-ups, process notes, and
        final frames. Check back shortly — or reach out for a private walkthrough.
      </p>
      <Link
        to="/"
        hash="contact"
        className="mt-10 inline-flex items-center gap-3 rounded-full border border-gold px-6 py-3 text-sm text-gold transition-colors hover:bg-gold hover:text-accent-foreground"
      >
        Request a walkthrough <ArrowUpRight className="h-4 w-4" />
      </Link>
    </div>
  );
}
